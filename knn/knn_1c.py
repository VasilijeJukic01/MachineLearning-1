...
plt.plot(X['petal_length'])
plt.plot(X['petal_width'])
plt.show()
...
X.loc[:, 'petal_length'] = scaler.fit_transform(X['petal_length'].values.reshape(-1, 1))
X.loc[:, 'petal_width'] = scaler.fit_transform(X['petal_width'].values.reshape(-1, 1))
...
for i in range(15):
    knn = KNN(3, X_train, y_train, i+1)
    accuracy_list.append(knn.accuracy({'x': X_test.values, 'y': y_test.values}))
...
sns.lineplot(x=ks , y=accuracy_list)
